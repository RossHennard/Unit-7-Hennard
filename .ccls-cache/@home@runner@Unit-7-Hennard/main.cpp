...
void GreenTea::setDelicious(bool d) { delicious = d; }
bool GreenTea::getDelicious() { return delicious; }
void GreenTea::setNameTea(string nt) { nameTea = nt; }
string GreenTea::getNameTea() { return nameTea; }
...
void Student::setLetterGrade(char lg) { letterGrade = lg; }
char Student::getLetterGrade() { return letterGrade; }
void Student::setPreference(GreenTea p) { preference = p; }
GreenTea Student::getPreference() { return preference; }
void Student::printTranscript() {
  cout << "\nName: " << name;
  cout << "\nID: " << id;
  cout << "\nGrade: " << letterGrade;
  cout << "\nTea: " << preference.getNameTea();

  if (preference.getDelicious() == true) {
    cout << "\nWell said, McMillanite, " << preference.getNameTea()
         << ", green tea is great!!!" << endl;
  } else {
    cout << "\n : '(" << endl;
  }
}
...
void fnClass(Student student) {
  ...
  cout << "\t" << student.getId() << ":" << student.getName()
       << " earned a(n): " << student.getLetterGrade();
...
void fnClassRef(Student &student) {
  cout << "\nA pass by REFERENCE object process (most effective):";
  char newGrade = '\0';
...
int main() {
  ...
  cout << "/nPrint Graduate Student" << endl;
  cout << "Start Data: ";
  graduate.printTranscript();

  cout << "\nI say again..." << endl;
  if (graduate.getPreference().getDelicious() == true) {
    cout << "\nWell said, McMillanite, "
         << graduate.getPreference().getNameTea() << ", green tea is great!!!"
         << endl;
  } else {
    cout << "\n : '(" << endl;
  }
